...
# nohup python3 -u trainSCID.py > result.log 2>&1 &
...
from utils.SCID.data import train_loader, val_loader, test_loader
...
file_handler = logging.FileHandler(os.path.join(case_dir, 'Case_SCID.log'))
...
model_pt_dir = os.path.join(os.path.join(os.path.dirname(__file__), 'pt_dir'), 'SCID')
assert os.path.isdir(model_pt_dir), f'File not found, {os.path.dirname(__file__)}'

test_images = []
read_path = os.path.join(os.path.join(os.path.join(os.path.abspath(os.path.join(os.getcwd(), "..")), 'utils'), 'SCID'), 'test.csv')
print(read_path)
...
for epoch in range(1, 121):
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(val_loader):
            ...
            with open('./model_result/SCID/val/result_{}.txt'.format(epoch), 'a') as file_handle:
                file_handle.write(
    ...
    with open('./model_result/SCID/test/result_{}.txt'.format(epoch), 'a') as file_handle:  # .txt可以不自己新建,代码会自动新建
        file_handle.write("{}\n".format("predscore" + " " + "GT"))
    net.eval()
    test_loss = 0.0
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(test_loader):
            ...
            with open('./model_result/SCID/test/result_{}.txt'.format(epoch), 'a') as file_handle:
                file_handle.write(
                    "{}\n".format(str(predsc_1[len(predsc_1) - 1]) + " " + str(gt_score_1[len(gt_score_1) - 1])))

        file_handle.close()
        write = pd.DataFrame(columns=col_result, data=result_data)
        write_path = os.path.join(os.path.dirname(__file__), 'SCID_TestResult.csv')
        ...
        write_path = os.path.join(os.path.dirname(__file__), 'SCID_ScoreData.csv')
        assert os.path.isfile(write_path), f'File not found, {write_path}'
        write.to_csv(write_path, mode='a', header=True, index=False)
        logger.info(
