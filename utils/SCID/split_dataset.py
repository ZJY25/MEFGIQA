...
dataset_DistortedImages_dir = os.path.join(dataset_root, 'DistortedSCIs')
...
save_dataset_root = os.path.join(
    os.path.join(os.path.abspath(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), 'DataSet'), 'SCID')
...
csv_data = pd.read_csv(scores_csv, sep=',', usecols=['image', 'mos', 'style', 'grade'])
...
for i in range(len(csv_data)):
    image_score_dict[csv_data.iloc[i].image] = csv_data.iloc[i].mos
...
test_images = []
total_images = []
read_path = os.path.join('/data2/cyk/MyIQA2New/622/SCID', 'image_p2.csv')  # 指定划分的文件image1
print(read_path)
